	...
	for a in get_children():
		if a is Attack and a.may_attack(from, to):
			...
			if a == selected_attack:
				print("%s attacking with %s at %f" % [from.display_name, a.get_display_name(), GameEngine.time_in_minutes])
				return true
	if selected_attack and new_attack:
		next_use_at = GameEngine.time_in_minutes + time_to_switch_in_mins
		print("%s waiting to switch to %s at %f" % [ from.display_name, new_attack.get_display_name(), next_use_at])
